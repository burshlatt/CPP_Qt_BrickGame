...
public:
 ...
 public:
  GameInfo_t GetGameInfo() const override { return game_info_; }

 public:
  void Stop() noexcept override { game_info_.pause = true; }

  void ResetState() override { Reset(); }
 ...
 private:
  ...
  void Reset() {
    dots_.clear();
    dots_ = {Coords(5, 10), Coords(5, 11), Coords(5, 12), Coords(5, 13)};
  ...
  void UpdateDirection(Direction direct) noexcept {
    switch (direct) {
      case Direction::kUp:
        if (direction_ != Direction::kDown) direction_ = direct;
        break;
      case Direction::kDown:
        if (direction_ != Direction::kUp) direction_ = direct;
        break;
      case Direction::kLeft:
        if (direction_ != Direction::kRight) direction_ = direct;
        break;
      case Direction::kRight:
        if (direction_ != Direction::kLeft) direction_ = direct;
        break;
    }
  }
  ...
  void MoveHead() {
    ...
    if (!is_walls_) СorrectMovement(head);
  }
  ...
  bool CheckCollide() {
    ...
    if (is_walls_) {
      ...
      if (is_collide) return is_collide;
    }
  ...
  void ClearField() noexcept {
    for (int i{}; i < GameInfo_t::field_rows; ++i)
      for (int j{}; j < GameInfo_t::field_cols; ++j) game_info_.field[i][j] = 0;
  }

  void MergeField() noexcept {
    for (const auto& [x, y] : dots_) game_info_.field[y][x] = 1;
