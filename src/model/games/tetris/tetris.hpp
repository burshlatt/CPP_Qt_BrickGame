...
class Tetris : public Game {
 ...
 private:
  ...
  void Move(Direction direct) {
    if (!game_info_.pause) MakeTransformations(direct);
  }
  ...
  bool CopyFigure() {
    for (auto [row, col] : game_info_.next.indices)
      if (game_info_.field[row][col] == 1) return false;
  ...
  bool MoveFigureDown() {
    for (auto [row, col] : game_info_.current.indices) {
      bool is_correct_move{row == GameInfo_t::field_rows - 1 ||
                           (game_info_.field[row + 1][col] == 1 &&
                            !IsAvailableIndex(row + 1, col))};

      if (is_correct_move) return false;
    }
  ...
  void Shift(Direction direct) {
    ...
    if (direct == Direction::kLeft) direction = -1;
  ...
  bool IsShiftAvailable(Direction direct) {
    ...
    for (auto [row, col] : game_info_.current.indices) {
      bool is_correct_move{col == last_col ||
